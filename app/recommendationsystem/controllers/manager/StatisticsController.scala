...
class StatisticsController extends Controller{
  ...
  private def calculateMax: Future[Option[(String, Int)]] = {
    ...
    TagsOdb.all flatMap { case tags =>
      val xs = tags map { case tag =>
        val tagName = "'"+ tag.flatten +"'"//create the String representation of the Tag.
      //Search all the document where tags.tag == tagName and for each document return a tuple (tag, number).
      val query = "select from Requests where outE(RequestsHoldsTag).inV(Tag) IN (select from Tags where tag in "+ tagName +" )"
  ...
  */
  ...
  private def calculateStatisticsOnRange(range: Range): Future[(Double, Int)] = {
    ...
    */
    Requests.find(query) flatMap {results => //callback on the find on the db
      if(results.size > 0) { //at least one document match the query
        val query1 : String = "Select from Advices where clicked='true' and date>="+range.startDate+" and date<="+range.finishDate
        Advices.find(query1) flatMap { elements => //finds all the Advices and set a callback on the query
...
*/
    ...
    private def calculateRequests(range: Range): Future[List[recommendationsystem.models.Request]] = {
      ...
      */
      val query : String = "select from Requests where date>="+range.startDate+" and date<="+range.finishDate
      Requests.find(query) flatMap {result => Future{result}
      }
    }
...
*/
    ...
    private def calculateTopAdvicesTagsInRange(range: Range) : Future[List[(String, Int)]] = {
      ...
      def findTag(tag: Tag): Future[(String, Int)] = {
        ...
        */
        val query : String = "select from Requests where date<="+range.startDate+ " and date>= "+range.finishDate+ " and outE(RequestHoldsTag).inV(Tags) IN (select from Tags where tag='" + tagName + "')"
        Requests.find(query) flatMap { results =>
          if(results.size > 0)
      ...
      val futureTags = Tags.all flatMap {tags =>
