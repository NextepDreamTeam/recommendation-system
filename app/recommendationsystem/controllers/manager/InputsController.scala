...
class InputsController extends Controller{
  ...
  def list(page: Int) = Action.async {
    val pageLength = 50	
    val futureRequests = Requests.all.sort(Json.obj("date" -> -1)).skip((page - 1) * pageLength).limit(pageLength).toList
    ...
    for {
      ...
      Ok(recommendationsystem.views.html.manager.inputs.list("test", requests, count, page, pageLength))
    }
  }
